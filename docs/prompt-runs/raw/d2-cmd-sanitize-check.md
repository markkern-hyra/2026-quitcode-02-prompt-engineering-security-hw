...
> **Версія команди.** Прогін виконано на версії `/sanitize-check` до першого рев'ю CodeRabbit, у якій `allowed-tools` дозволяв `grep`, `cut`, `paste`, `wc`. Поточна версія працює лише через `docs/tools/sensitive-lines.sh` — див. прогін [`e5-cmd-sanitize-check-wrapper`](./e5-cmd-sanitize-check-wrapper.md). Цей лог не переписується: він фіксує, що саме тоді виконала модель.
