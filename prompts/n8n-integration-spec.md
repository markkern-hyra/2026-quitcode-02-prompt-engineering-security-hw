...
tested-on: docs/sanitized-brief.md (до і після виправлення) — claude -p (Opus 4.5, лише читання), v1–v4, 2026-09-14; лог — docs/prompt-runs/06-n8n-integration-spec.md
version: 4
---
...
| v4 | ✅ усі 9 нових критеріїв: позначені нові credentials, секрет лише в заголовку, `event_id` з атомарною дедуплікацією, порядок перевірок до будь-яких записів і логів, HTTPS і редиректи, ідемпотентний CRM, серверний `receivedAt`, TTL + шифрування + окремі credentials сховищ, черга як `ПРИПУЩЕННЯ`; «9 цифр» узгоджено скрізь | дрібне: не кожен новий credential окремо винесено у відкриті питання |
...
- **v4** — за другим раундом рев'ю CodeRabbit на вихід v3: нові credentials
  (`WEBHOOK_HMAC_SECRET`, `RATE_LIMIT_STORE`) з'явились без позначки; «shared
  secret у query-параметрі» як запасний варіант; захист від повторів лише
  вікном timestamp без ID події; порядок перевірок не охоплював сховища й логи;
  жодних вимог до HTTPS і редиректів для вихідних викликів; повтори CRM без
  ідемпотентності; сховище збоїв зі спільним credential, локальний резерв без
  строку зберігання; «10 цифр» у тексті при 9 у regex; SLA без явного серверного
  часу; висновок «черга не потрібна» як факт. Додано відповідні критерії й
  розділ формату «Безпека вихідних викликів».
