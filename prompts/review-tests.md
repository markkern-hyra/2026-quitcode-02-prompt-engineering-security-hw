...
tested-on: app/src/quote.test.ts (41 тест після add-tests + debug) — claude -p (Opus 4.5), v1 і v3, 2026-09-14; лог — docs/prompt-runs/03-review-tests.md
version: 3
---
...
| 2026-09-14 (v3) | ✅ перевірка `git status` перед мутаціями, 8 мутацій із колонкою «стан після відновлення: чистий», фінальний стан чистий, 0 `git checkout`; знайдено **всі три** дублікати | пропущено оманливу назву «33.33...%», яку знайшов v1 |

## Журнал змін

- **v3** — за другим раундом рев'ю CodeRabbit: відновлення обов'язкове за будь-якого
  результату мутації, перевірка чистого стану після кожної мутації, негайна
  зупинка, якщо відновити не вдалося. Перезапущено (див. «Прогони»).
